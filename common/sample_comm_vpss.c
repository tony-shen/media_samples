...
RK_S32 SAMPLE_COMM_VPSS_CreateChn(SAMPLE_VPSS_CTX_S *ctx) {
	...
	s32Ret = RK_MPI_VPSS_SetGrpRotation(ctx->s32GrpId, ctx->s32RotationEx);
	if (s32Ret != RK_SUCCESS) {
		RK_LOGE("RK_MPI_VPSS_SetGrpRotation set Grp Rotation Failure!");
		return s32Ret;
	}
	...
	s32Ret = RK_MPI_VPSS_EnableBackupFrame(ctx->s32GrpId);
