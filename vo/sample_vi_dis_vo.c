...
static RK_CHAR optstr[] = "?::a::w:h:d:D:I:L:M:r:";
static const struct option long_options[] = {
    {"aiq", optional_argument, NULL, 'a'},
    {"device_name", optional_argument, NULL, 'd'},
...
static void print_usage(const RK_CHAR *name) {
	...
	printf("\t%s -w 2560 -h 1520 -a /etc/iqfiles/ -I 0 -D 0 -L 0\n", name);
#ifdef RKAIQ
	...
	printf("\t-p | --pixel_format: camera Format, Default nv12, "
		"Value:nv12, nv12 afbc.\n");
	printf("\t-D | --disp_devid: display DevId, Default 0\n");
	printf("\t-L | --disp_layerid: display LayerId, Default 0\n");
	printf("\t-r | --vo_use_rga: whether vo composer using RGA, Default 1:RGA, 0:GPU\n");
}
...
int main(int argc, char *argv[]) {
	...
	while (!quit) {
		sleep(10);
		s32Ret = RK_MPI_VI_GetChnDISAttr(ctx->vi.s32DevId, ctx->vi.s32ChnId, &pstDisAttr);
		if (s32Ret != RK_SUCCESS) {
			RK_LOGE("vi [%d, %d, %d] RK_MPI_VI_GetDisAttr failed with %#x!",
				ctx->vi.s32DevId, ctx->vi.u32PipeId,
				ctx->vi.s32ChnId, s32Ret);
			return s32Ret;
		}

		RK_LOGV("VI [%d %d %d] change dis config bStillCrop (%d -> %d)",
			ctx->vi.s32DevId, ctx->vi.u32PipeId,
			ctx->vi.s32ChnId,
			pstDisAttr.bStillCrop, !pstDisAttr.bStillCrop);
		pstDisAttr.bStillCrop = !pstDisAttr.bStillCrop;

		s32Ret = RK_MPI_VI_SetChnDISAttr(ctx->vi.s32DevId, ctx->vi.s32ChnId, &pstDisAttr);
		if (s32Ret != RK_SUCCESS) {
			RK_LOGE("vi [%d, %d, %d] RK_MPI_VI_SetDisAttr failed with %#x!",
				ctx->vi.s32DevId, ctx->vi.u32PipeId,
				ctx->vi.s32ChnId, s32Ret);
			return s32Ret;
		}
