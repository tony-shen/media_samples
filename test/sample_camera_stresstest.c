...
static void *vpss_get_stream(void *pArgs) {
	...
	while (!quit) {
		...
		if (s32Ret == RK_SUCCESS) {
			...
			s32Ret = RK_MPI_VENC_SendFrame(
			    g_ctx->venc.s32ChnId,
			    (VIDEO_FRAME_INFO_S *)&ctx->stChnFrameInfos.stVFrame, -1);
			if (s32Ret != RK_SUCCESS) {
...
int SAMPLE_CAMERA_ISP_Stresstest(SAMPLE_MPI_CTX_S *ctx, char *pIqFileDir) {
	...
	if (iq_file_dir) {
		...
		if (s32Ret != RK_SUCCESS) {
			printf("%s : isp init or run failure\n", __func__);
			return RK_FAILURE;
		}
#endif
	}
...
int SAMPLE_CAMERA_VENC_Stresstest(SAMPLE_MPI_CTX_S *ctx, RK_S32 mode) {
	...
	if (iq_file_dir) {
		...
		s32Ret |= SAMPLE_COMM_ISP_Run(s32CamId);
		if (s32Ret != RK_SUCCESS) {
			printf("%s : isp init or lunch failure\n", __func__);
			return RK_FAILURE;
		}
#endif
	}
