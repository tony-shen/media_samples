...
static RK_CHAR optstr[] = "?::a::b:w:h:l:o:e:d:I:M:f:F:";
static const struct option long_options[] = {
    ...
    {"srcfps", required_argument, NULL, 'f'},
...
static void print_usage(const RK_CHAR *name) {
	...
	printf("\t-f | --set venc input fps, Default 25\n");
...
int main(int argc, char *argv[]) {
	...
	if (iq_file_dir) {
		...
		if (s32Ret != RK_SUCCESS) {
			printf("%s : isp init or start failure\n", __func__);
			return RK_FAILURE;
		}
#endif
	}
	...
	ctx->vi.s32ChnId = 0;
	ctx->vi.stChnAttr.stIspOpt.u32BufCount = 10;
	...
	ctx->venc.u32SrcFps = s32SrcFps;
	ctx->venc.u32DstFps = s32DstFps;
	ctx->venc.u32Gop = 50;
